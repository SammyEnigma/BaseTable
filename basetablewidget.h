 ...
 *@date:   2016.8.1
 *@brief:  构建自定义Table组件
 *@update: 2016.9.12  对于单元格控件对象不再用数组单独保存，省去删除操作所带来的
 * 数组移动时间，对于控件的获取直接用cellwidget（）多个部件使用findchild（）
 * 2018.8.8   对一些细节优化，添加一些新的功能
 * 2018.8.10   添加委托类接口
 */
...
public:
    ...
    explicit BaseTableWidget(int rowNum,int columnNum,QList <BaseTableItem *> items,bool rowInit=true,QWidget *parent = 0);
